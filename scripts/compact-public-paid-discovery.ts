...
const KEEP_OPENAPI_PATHS = new Set([
  ...
  "/api/base-usdc-settlement-verify",
  "/api/payment-guard",
...
openapi.info = {
  ...(openapi.info ?? {}),
  description:
    "Focused machine catalog for x402 settlement testing and payment verification for autonomous buyers, including pre-payment verification and post-payment Base USDC receipt verification. Use GET /api/x402-ping for a $0.001 USDC settlement canary. Before an autonomous x402 purchase, use AgentResolver Guard via GET /api/payment-guard or canonical GET /api/x402-payment-preflight for a $0.001 fail-closed payment-path check. After a Base USDC transfer, use GET /api/base-usdc-settlement-verify for $0.001 transaction-level settlement evidence. Exact buyer-intent Guard aliases remain published, while unrelated paid utilities stay live but are omitted from public machine discovery to reduce unpaid crawler sweeps. Free capability discovery remains available through POST /api/resolve.",
  "x-guidance":
    "Prefer the shortest paid funnel: GET /api/x402-ping -> GET /api/payment-guard or GET /api/x402-payment-preflight before spend -> GET /api/base-usdc-settlement-verify after a Base USDC payment -> POST /api/verified-resolve or /api/batch-verified-resolve when capability selection is needed. Exact Guard buyer-intent aliases remain discoverable. Public discovery intentionally excludes unrelated paid utilities even though those routes remain live. Free capability discovery remains available through POST /api/resolve. Payment remains caller-authorized."
};
