...
export async function verifiedResolve(goal: string, url?: string) {
  ...
  const verifiedMcp = probes
    ...
    .sort((a, b) => (b.confidence - a.confidence) || ((a.latencyMs ?? Infinity) - (b.latencyMs ?? Infinity)));
  ...
  return {
    ...
    verificationPolicy: {
      ...
      discoverySources: ["AgentResolver owned catalog", "official MCP registry", "mcpub", "x402 marketplace"],
      callerSpendingAuthorized: false,
      note: "This operation verifies a bounded set of top MCP candidates. Returned candidates still require the caller's own trust, privacy, and permission checks before use."
    },
    owned: resolution.owned,
    marketplace: resolution.marketplace,
    mcp: resolution.mcp,
    liveVerification: probes,
    recommendation: verifiedMcp.length > 0
      ? {
          type: "verified-mcp",
          endpoint: verifiedMcp[0].endpoint,
          name: verifiedMcp[0].name,
          reason: "Top discovered MCP candidate completed a live initialize + tools/list verification."
        }
      : resolution.owned.length > 0
        ? {
            type: "owned-capability",
            capability: resolution.owned[0],
            reason: "No probed MCP candidate completed verification; highest-ranked live AgentResolver-owned capability is shown."
          }
        : {
            type: "discovery-only",
            reason: "No candidate completed live MCP verification. Review the returned directory and marketplace candidates."
          }
  };
}
