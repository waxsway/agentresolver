...
export function x402ChallengeHeaderHandoff(capabilityId: string) {
  const info = {
    ...
    clients: {
      ...
      managedWalletMcp: {
        install: X402_CHALLENGE_CLIENT_INSTALLS.managedWalletMcp,
        package: X402_CHALLENGE_CLIENT_ENTRYPOINTS.managedWalletMcp.package,
        command: X402_CHALLENGE_CLIENT_ENTRYPOINTS.managedWalletMcp.command,
        tools: X402_CHALLENGE_CLIENT_ENTRYPOINTS.managedWalletMcp.tools,
        network: X402_CHALLENGE_CLIENT_ENTRYPOINTS.managedWalletMcp.defaultNetwork
      }
    }
  } as const;
