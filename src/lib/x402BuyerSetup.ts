...
export function normalizeX402ChallengeResumeUrl(
  ...
  if (!requestUrl || !endpoint || requestUrl.length > 4096) return null;
  try {
...
export function x402BuyerSetupChallengeUrl(
  capabilityId: string,
  method?: string | null,
  ...
  if (challengeMethod) params.set("method", challengeMethod);
  if (resumeUrl) params.set("resumeUrl", resumeUrl);
  return `${X402_BUYER_SETUP_URL}?${params.toString()}`;
}

export function x402BuyerSetupChallengeError(
  capabilityId: string,
  method?: string | null,
  resumeUrl?: string | null
) {
  return `Payment required. x402 buyer setup: ${x402BuyerSetupChallengeUrl(capabilityId, method, resumeUrl)}`;
}
...
export function x402ChallengeBuyerHandoff(
  capabilityId: string,
  method?: string | null,
  resumeUrl?: string | null
) {
...
export function x402ChallengeHeaderHandoff(
  capabilityId: string,
  method?: string | null,
  resumeUrl?: string | null
) {
  ...
  const challengeContext =
    context.source === "x402-challenge" && context.capabilityId
      ? {
          ...
          resumeUrl:
            normalizeX402ChallengeResumeUrl(context.resumeUrl, context.endpoint) ??
            (context.endpoint ? `https://agentresolver.vercel.app${context.endpoint}` : null),
    ...
    clients: {
      ...
      coinbaseAgentKit: {
        ...
        discovery: {
          facilitator: "payai",
          reason:
            "AgentKit defaults discover_x402_services to CDP, while AgentResolver's currently verified Bazaar listing is on PayAI.",
          maxUsdcPrice: 0.001,
          x402Versions: [2],
          keyword: null,
          ...
          rule:
            "Pass facilitator=payai explicitly for AgentResolver discovery; do not rely on AgentKit's CDP default. After discovery, keep the normal caller-controlled service approval and spend authorization flow."
        },
        serviceApproval: {
        ...
        paymentGuardUrl: "https://agentresolver.vercel.app/api/payment-guard",
        behavior: [
          "For AgentResolver discovery, call discover_x402_services with facilitator=payai, maxUsdcPrice=0.001, x402Versions=[2], and no keyword filter. AgentKit defaults discovery to CDP, so omitting facilitator can miss AgentResolver's verified PayAI Bazaar canary.",
