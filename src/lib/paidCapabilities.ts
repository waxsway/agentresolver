...
export const PAID_CAPABILITIES = {
  ...
  "x402-payment-preflight": {
    ...
    costClass: "bounded-network",
    tags: ["x402 preflight", "verify endpoint before paying", "payTo verification", "USDC payment check", "bounded preflight", "no private keys", "deployment provenance", "request receipt", "resource binding", "Base mainnet", "Solana", "payment-required", "agent payment safety"],
    inputSchema: {
      type: "object",
      required: ["url"],
      additionalProperties: false,
    ...
    quoteTool: {
      ...
      description: "Paid $0.001 USDC on Base or Solana bounded x402 preflight. No private key or target payment is requested. Verify payTo, quote, network, asset, resource binding and live challenge evidence before agent spend; response headers include request correlation and exact deployment provenance."
    }
  },
  "tool-contract": {
    id: "tool-contract",
