...
function getPaidHandler(): PaidHandler {
  ...
  paidHandler = withX402<unknown>(auditHandler, { "/api/agent-readiness": { accepts: { scheme: "exact", price: X402_PRICING.agentReadiness, network: X402_NETWORK, payTo: payTo as `0x${string}` }, description: "Audit a public website for AI-agent discoverability and machine-readable integration signals.", mimeType: "application/json" } }, resourceServer) as PaidHandler;
  return paidHandler;
}
