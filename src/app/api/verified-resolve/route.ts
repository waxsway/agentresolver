...
async function verifiedResolveHandler(req: NextRequest): Promise<NextResponse<unknown>> {
  ...
  if (!goal) {
    return NextResponse.json(
      { error: "MISSING_GOAL", message: "Provide the capability you need verified." },
      { status: 400 }
    );
  }
  if (goal.length > MAX_GOAL || (url && url.length > MAX_URL)) {
    return NextResponse.json(
      { error: "INPUT_TOO_LONG", message: "Goal or target URL is too long." },
      { status: 400 }
    );
  }
  ...
  console.log(
    JSON.stringify({
      ...
      verifiedMcpCount: report.liveVerification.filter((item) => item.mcpCompatible).length,
      candidateCount:
        report.owned.length + report.mcp.length + report.marketplace.length
    })
  );
...
function getPaidHandler(): PaidHandler {
  ...
  paidHandler = withX402<unknown>(
    verifiedResolveHandler,
    {
      "/api/verified-resolve": {
        accepts: {
          scheme: "exact",
          price: X402_PRICING.verifiedResolve,
          network: X402_NETWORK,
          payTo: payTo as `0x${string}`
        },
        description:
          "Resolve a missing capability and live-verify up to two top MCP candidates before returning a recommendation.",
    ...
    return NextResponse.json(
      {
        ...
        capabilityId: "verified-resolve",
        message: "Verified Resolve is temporarily disabled."
      },
      { status: 503 }
    );
  }
