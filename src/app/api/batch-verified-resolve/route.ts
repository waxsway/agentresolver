...
async function batchHandler(req: NextRequest): Promise<NextResponse<unknown>> {
  ...
  if (!Array.isArray(body?.items) || body.items.length === 0) {
    return NextResponse.json(
      { error: "MISSING_ITEMS", message: "Provide 1 to 4 capability requests in items[]." },
      { status: 400 }
    );
  }

  if (body.items.length > MAX_GOALS) {
    return NextResponse.json(
      { error: "TOO_MANY_ITEMS", message: "A batch supports at most 4 capability requests." },
      { status: 400 }
    );
  }
  ...
  if (items.some((item) => !item.goal)) {
    return NextResponse.json(
      { error: "MISSING_GOAL", message: "Every item must include a non-empty goal." },
      { status: 400 }
    );
  }
  ...
  if (
    items.some(
      (item) =>
        ...
        (item.url && item.url.length > MAX_URL_LENGTH)
    )
  ) {
    return NextResponse.json(
      { error: "INPUT_TOO_LONG", message: "A goal or target URL exceeds the allowed length." },
      { status: 400 }
    );
  }
  ...
  console.log(
    JSON.stringify({
      ...
      maxProbeBudget: report.limits.maxLiveMcpProbesTotal
    })
  );
  ...
  paidHandler = withX402<unknown>(
    batchHandler,
    {
      "/api/batch-verified-resolve": {
        accepts: {
          scheme: "exact",
          price: X402_PRICING.batchVerifiedResolve,
          network: X402_NETWORK,
          payTo: payTo as `0x${string}`
        },
        description:
          "Resolve and live-verify up to four capability requests in one paid batch, probing up to two MCP candidates per request.",
...
export async function POST(req: NextRequest) {
  if (process.env.BATCH_VERIFIED_RESOLVE_ENABLED === "false") {
    return NextResponse.json(
      {
        error: "CAPABILITY_NOT_LIVE",
        capabilityId: "batch-verified-resolve",
        message: "Batch Verified Resolve is temporarily disabled."
      },
      { status: 503 }
    );
  }
