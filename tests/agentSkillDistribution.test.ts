...
test("installable and published Guard skills carry the case-safe MCP pre-sign gate", () => {
  for (const text of [installable, published]) {
    ...
    assert.match(text, /context\.paymentRequired\.x402Version !== 2/);
    assert.match(text, /context\.paymentRequired\.resource\.url !== expectedResource/);
    assert.match(text, /requirement\.amount === "1000"/);
    assert.match(text, /requirement\.asset === expected\.asset/);
    assert.match(text, /requirement\.payTo === expected\.payTo/);
    assert.match(text, /Solana Base58 identifiers are case-sensitive/i);
    assert.doesNotMatch(text, /requirement\.payTo\.toLowerCase/);
  }
});
